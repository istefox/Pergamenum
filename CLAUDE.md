...
- **ADR-0057** — Closes `PG-153`/#262, a window ADR-0043's implementation notes filed as «wider than §D8 can close»: the diary gains its own origin marker and one guarded write door, the ADR-0052/ADR-0054 pattern applied to a third in-memory holder. `readDiary` now returns the disk state it read (present-with-hash or absent), `writeDiary(over:)` takes that state as a mandatory precondition instead of writing unconditionally, and a writer landing in between is refused — surfaced as a non-modal `.conflicted` `saveState` with «Mantieni le mie modifiche» / «Ricarica dal disco», never a silent overwrite or a silent drop. All writes serialize through one door (`enqueue`/`drain`/`runner`), so a day switch, a keystroke-triggered save and an app-quit flush can never race each other in-process. Deliberately scoped out and filed as its own four issues (§D8): five other read-then-write sites still write unguarded (#496), the quit flush is fire-and-forget (#497), the Diario pane learns of an external change only when it next tries to write (#495), and `WorkspaceController` silently discards a conflicted board on navigation (#498) — all four closed by ADR-0058. No on-disk format, schema or protected interface touched → `docs/adr/0057-diary-origin-marker-and-one-write-door.md`
- **ADR-0058** — Closes ADR-0057 §D8's four named follow-ups (#495, #496, #497, #498). Five more read-then-write sites in `Sources/Vault` (`append`, `linkFromDailyNote`, the `addStructuralLink` pair, `moveOnBoard`, `captureTask`'s creation case) adopt the same `expecting:`/`expectingAbsent:` guard the diary already had; `addStructuralLink`'s doc comment is corrected from claiming full atomicity to the weaker guarantee the guard actually gives (a refusal on the second write leaves the first landed, named rather than rolled back). `AppDelegate.applicationShouldTerminate(_:)` now awaits a new `DiaryController.settle()` (capped at 2s) before quitting, replacing a `willTerminateNotification` flush nothing awaited. `VaultController` gains `didObserveExternalChange`, fanned out from `reconcile` alongside the existing tab fan-out and wired to a new `DiaryController.handleExternalChange(path:text:)` — the Diario pane reloads on a clean external change and enters conflict on a dirty one, the same turn the watcher sees it. `WorkspaceController.load(board:)`/`select(_:)` gain `refusesToLeaveConflictedBoard()`, closing two defects found during implementation beyond the original four: `select(nil)` used to wipe a conflicted board's `document`/`origin` while leaving `saveState == .conflicted`, making the state unrecoverable; and the sidebar's rename/move/trash verbs (`WorkspaceView+FolderVerbs.swift`) still performed their disk operation on a conflicted open board, stranding the conflict on a dead path — both closed by the same guard, exposed as `canLeaveOpenBoardForVerb()` for the verb call sites. `WorkspaceController`'s own quit-flush gap, the same shape as #497 but for boards, is named and filed separately (#506) rather than folded in. No on-disk format, schema or protected interface touched. Extends ADR-0057, amends none → `docs/adr/0058-adr0057-followups-five-write-sites-quit-flush-diary-reload-board-navigation.md`
