...
struct PergamenumApp: App {
    ...
    init() {
        ...
        // ADR-0057 §D8, #495: an external change reaches the Diario pane too, not only the
        // editor's tabs - the diary is the one in-memory holder of a `.md` outside them.
        vault.didObserveExternalChange = { [weak diary] path, text in
            diary?.handleExternalChange(path: path, text: text)
        }
