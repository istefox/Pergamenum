...
extension VaultSession {
    ...
    func eventNote(
        ...
        return CreatedEventNote(
            path: relativePath, dailyNote: linkFromDailyNote(to: title, on: day)
        )
    }
    ...
    private func linkFromDailyNote(to title: String, on day: CalendarDate) -> WriteResult? {
        do {
            let relativePath = try dailyNote(for: day)
            let text = try read(relativePath).text
            let updated = EventNoteSection.adding(title, to: text)
            guard updated != text else { return nil }
            return try write(updated, to: relativePath)
        } catch {
