# ADR-0058: ADR-0057's four named follow-ups — five write sites, quit flush, diary reload, board navigation
...
each one. They were promoted to GitHub issues (#495, #496, #497, #498) and are what this
chain closes.

Exploration corrected one premise stated in #495's own text. `VaultWatcher`'s filter
(`Sources/Vault/VaultWatcher.swift:63-81`) accepts any `.md` outside a dot-directory, and a
diary day file is `Diario/20260814.md` — it *is* delivered. The real gap was on the consumer
side: `VaultController.reconcile` fanned out only to editor tabs, and `DiaryController` is
not a tab. The fix is wiring, not filtering.
...
(ADR-0046 §D3's rule, applied here).
...
### D3 · #495 — wire the watcher through to the Diario pane

`VaultController` gains `didObserveExternalChange: ((String, String) -> Void)?`, called from
`reconcile` for every change alongside the existing `updateTabs(showing:)`, wired in
`PergamenumApp.init` to `DiaryController.handleExternalChange(path:text:)`.

`handleExternalChange` mirrors the tab reconciliation rule: ignored for any day but the one
open, ignored while already `.conflicted` (the conflict already pins the pane), and ignored
when the incoming text's hash already matches what `origin` says the file holds (a self-write
echo — `VaultSession.reconcile` already prunes `selfWrittenHashes`, so this is a second,
cheap guard rather than the only one). Otherwise: nothing owed → `reload()`, re-reading from
disk and re-origining, so a clean pane picks up the change at once; something owed and no
write in flight → `enterConflict()`, the same "never merge, never discard: ask" rule the tab
path uses; something owed with a write queued or running → nothing here, since that write
already carries the precondition and will refuse into the same conflict on its own — entering
it from here could be undone by a write that lands before the change is even applied.

### D4 · #498 — refuse navigation away from a conflicted board
...
- A quit with unsaved diary text now waits up to 2 seconds for the write rather than losing it
  outright.
- The Diario pane reacts to an external change the same turn the watcher sees it, instead of
  only on its own next write attempt.
...
- New tests: `Tests/VaultUnguardedWriteGuardTests.swift`, `Tests/DiaryExternalChangeTests.swift`,
  `Tests/DiarySettleTests.swift`, plus two additions to `Tests/WorkspaceAutosaveRaceTests.swift`.
