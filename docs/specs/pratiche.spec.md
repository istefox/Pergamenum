...
- [ ] R-10 — Attachments are copied into `allegati/` as `YYYYMMDD_<name>`; an attachment whose SHA-256 already exists in the pratica is linked, not copied; one above the threshold (default 100 MB) is recorded as a store reference without copy; an inline image light in both bytes (under 50 KB) and pixel dimensions (≤200×200) is dropped, otherwise (heavy, bigger than 200×200, or with unreadable dimensions) saved and embedded. A part whose inline MIME payload decodes to zero bytes is tried once more against Mail's own sibling `Attachments/<ROWID>/<part>/` directory (`EMLXReader.attachmentsDirectory`, keyed by the part's real IMAP-style RFC 3501 number) before it is treated as not-yet-downloaded — Exchange sometimes externalizes a part's bytes there permanently rather than leaving them inline (ADR-0048).
