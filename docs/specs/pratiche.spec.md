...
- Inline image under 50 KB: dropped (signatures, logos). Over: saved and embedded as `![[…]]`.
...
- [ ] R-10 — Attachments are copied into `allegati/` as `YYYYMMDD_<name>`; an attachment whose SHA-256 already exists in the pratica is linked, not copied; one above the threshold (default 100 MB) is recorded as a store reference without copy; inline images under 50 KB are dropped and larger ones saved and embedded.
