...
- **Rinomina ed eliminazione cartella**: dalla barra strumenti del sidebar del Workspace è possibile rinominare o eliminare la cartella selezionata (radice esclusa). Una rinomina ripunta i percorsi `.canvas` dei nodi in tutto il vault e, quando il nome del file board non è ambiguo tra più cartelle, riscrive anche il marcatore `^[[vecchio.canvas]]` che apre quella board da una nota — un wikilink `[[Nota]]` ordinario non viene mai toccato, perché nomina una nota per titolo, non per percorso. L'eliminazione sposta l'intera cartella nel Cestino di sistema (`FileManager.trashItem`, mai `removeItem`); se la board aperta è la cartella eliminata (o un suo antenato), il Workspace risale alla cartella padre superstite più vicina. Nessuna delle due operazioni è annullabile tramite il journal delle scritture: il ripristino passa dal Cestino o da una rinomina inversa.
