...
@Suite(.serialized) struct PraticaLedgerFolderTrashEdgeTests {
    ...
    @Test func aRefusedDeletionThroughItsOwnCommandKeepsTheKeyTheSelectionAndTheFolder() async throws {
        ...
        pratiche.ledger.byPraticaPath[path] = stateImporting("<a@rossi-spa.it>")
    ...
    /// in-memory state is still cleaned. A non-empty DIRECTORY at the ledger's own URL makes
    /// the atomic replace fail for certain, whatever the account or the volume.
    @Test func aLedgerThatCannotBeSavedIsReportedAndStillForgottenInMemory() async throws {
        ...
        try Data("x".utf8).write(to: url.appending(path: "blocker", directoryHint: .notDirectory))
    ...
    @Test func forgettingTheSamePathTwiceIsHarmless() {
        let pratiche = PraticheController(probe: { .granted }, performSync: { _, _ in })
        pratiche.ledger.byPraticaPath["01 Progetti/Tifone"] = stateImporting("<a@rossi-spa.it>")
        pratiche.ledger.byPraticaPath["01 Progetti/Verdi"] = stateImporting("<b@rossi-spa.it>")
